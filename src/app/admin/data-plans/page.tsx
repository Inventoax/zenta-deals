...
export default function AdminDashboardPage() {
  ...
  const [validity, setValidity] = useState("Non Expiry");
  ...
  return (
    <div className="p-8 bg-[#0f172a] min-h-screen text-white font-sans">
      <h1 className="text-3xl font-bold mb-8">Admin Dashboard: Package Manager</h1>
      ...
      <form onSubmit={handleAddPlan} className="bg-gray-800/50 border border-gray-700 p-6 rounded-2xl mb-10 max-w-2xl shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label className="block text-sm text-gray-300 mb-1">Data Amount in MB</label>
            <input 
              type="text" 
          ...
          <div>
            <label className="block text-sm text-gray-300 mb-1">Validity</label>
            <input 
              type="text" 
              placeholder="e.g. Non Expiry" 
              value={validity} 
          ...
          <div className="md:col-span-2">
            ...
            <input 
              type="text" 
              placeholder="e.g. Works on all devices" 
              value={description} 
          ...
          <div>
            <label className="block text-sm text-gray-300 mb-1">Customer Price (₵)</label>
            <input 
              type="number" 
              step="0.01" 
              placeholder="15.00" 
          ...
          <div>
            <label className="block text-sm text-gray-300 mb-1">Agent Price (₵)</label>
            <input 
              type="number" 
              step="0.01" 
              placeholder="12.50" 
      ...
      <h2 className="text-xl font-bold mb-4">Manage Existing Packages</h2>
        ...
        {plans.map((plan) => (
          <div key={plan.id} className="bg-gray-800/50 border border-gray-700 p-5 rounded-xl shadow-md flex flex-col justify-between">
            <div>
              ...
              <p className="text-gray-400 text-xs mb-2">Network: {plan.network} | Validity: {plan.validity}</p>
              {plan.description && <p className="text-gray-500 text-xs mb-4 italic">{plan.description}</p>}
